...
**ГОТОВО (2026-08-20):** все 4 ящика подтверждены рабочими на Windows через workspace-mcp — авторизация и `list_gmail_labels` прошли успешно для всех:

| Ящик | Меток | Статус |
|---|---|---|
| chegorx@gmail.com | 24 | ✅ |
| motus.cy@gmail.com | 22 | ✅ |
| autoexpertt21@gmail.com | 24 | ✅ |
| autoexpert.cy@gmail.com | 26 | ✅ |

Токены закэшированы на Windows-машине (`~\.google_workspace_mcp\credentials\`), повторная авторизация в браузере не требуется.

**Ключевые уроки этой установки (для повтора на третьей машине в будущем):**
1. Запускать `uv run main.py --transport streamable-http` — НЕ `uv run workspace-mcp` (тот стартует в режиме stdio).
2. `.env`, созданный через PowerShell `Set-Content -Encoding UTF8`, может содержать BOM (метку порядка байт), которая ломает имя ПЕРВОЙ переменной в файле (`GOOGLE_OAUTH_CLIENT_ID`) — сервер тогда не находит её, хотя файл выглядит заполненным. Писать `.env` через `[System.IO.File]::WriteAllText(...,(New-Object System.Text.UTF8Encoding($false)))` (без BOM) либо вручную через Блокнот.
3. После любого изменения `.env` — обязательно перезапускать процесс сервера (`.env` читается только при старте, не на лету).
4. Реальная диагностика оказалась возможна только через **настоящий локальный Claude Code**, установленный прямо на Windows (`npm install -g @anthropic-ai/claude-code`) — до этого несколько "Windows-сессий" оказывались облачными контейнерами без доступа к диску, что и было причиной почти всех прошлых тупиков.

Инструкция ниже (скрипт + текстовый вариант) сохранена как справочная для повтора на новых машинах.
...
### Готовый скрипт (рекомендуется — без Claude-посредника)

`scripts/windows-workspace-mcp-setup.ps1` в этом репозитории. Проверено: обе "Windows-сессии" Claude, через которые пытались это сделать 2026-08-18, оказались облачными контейнерами без доступа к реальному диску (см. `docs/sync-test.md` Round 2/3 — Round 3 ошибочно сообщил, что он локальный). Поэтому единственный надёжный путь — настоящий локальный Windows Terminal/PowerShell, без Claude между пользователем и командами.

Как использовать: скачать/скопировать этот `.ps1`-файл на Windows-машину, открыть **настоящий** Windows Terminal (не через VSCode, если тот подключён к облаку/devcontainer), вставить содержимое файла или запустить `powershell -File windows-workspace-mcp-setup.ps1`. Скрипт сам проверяет Python/git/uv, клонирует репозиторий, создаёт `.env` (секреты — пустые, вписать вручную) и безопасно дополняет `.mcp.json`, не перезаписывая существующее содержимое. В конце выводит точный путь к `.env` и команду для запуска сервера.

### Инструкция текстом (альтернатива — если решите делать через Claude-сессию)
