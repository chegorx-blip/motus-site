...
**В работе (пересмотрено 2026-08-18, позже в тот же день):** ранее в этот же день было временное решение остаться на 1 общем ящике + 3 только на Mac. Пользователь передумал — цель снова "все 4 ящика на обеих машинах". Второй компьютер — реальный Windows-ноутбук с локальным Claude Code (терминал VSCode, сессия подтверждена в Round 3 `docs/sync-test.md`), технически способен на то же самое, что и Mac. Инструкция ниже — актуальный план к выполнению.
